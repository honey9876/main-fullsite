...
import { ScrollTrigger } from "gsap/ScrollTrigger";
import phoneIllustration from "../assets/phone1.png";
import myVideo from "../assets/myVideo6.mp4";
...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    tl.fromTo(
      smartLettersRef.current,
      { x: () => gsap.utils.random(30, 80), opacity: 0, scale: 0.9 },
      {
        x: 0,
        opacity: 1,
        scale: 1,
        duration: 0.5,
        stagger10: { amount: 0.15 },
        ease: "power2.out",
      },
      "-=0.4"
    );
    ...
    tl.fromTo(
      growLettersRef.current,
      { y: () => gsap.utils.random(-80, -20), opacity: 0 },
      {
        y: 0,
        opacity: 1,
        duration: 0.5,
        ...
        <div className="text-center lg:text-left w-full max-w-full sm:max-w-md lg:max-w-xl space-y-3 sm:space-y-4 z-10 px-2 sm:px-0">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold text-[#4a3728] leading-tight font-aquire-bold -ml-4 sm:-ml-6 md:-ml-8 lg:-ml-16 xl:-ml-20">
            <span className="block">
              {splitTextToLetters("Network", networkLettersRef)}{" "}
              {splitTextToLetters("Smarter", smartLettersRef)}
            </span>
            <span className="block">
              {splitTextToLetters("Grow", growLettersRef)}{" "}
              {splitTextToLetters("Faster", fasterLettersRef)}
            </span>
          </h1>
          <p
            ref={subTextRef}
          ...
          <button
            ref={buttonRef}
            className="bg-[#a8744f] hover:bg-[#905f3b] text-white font-aquire-bold text-xs sm:text-sm md:text-base lg:text-lg px-4 sm:px-6 py-2 sm:py-3 rounded-full shadow-lg transition-all duration-300 "
            onClick={playSound}
          >
